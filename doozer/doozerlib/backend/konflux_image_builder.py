        ...
        try:
            ...
            for attempt in range(retries):
                ...
                with client.ApiClient(configuration=cfg) as api_client:
                    ...
                    self._logger.info("[%s] PipelineRun %s completed with status %s",
                                      metadata.distgit_key, pipelinerun_name, status)
                    ...
                    if status != "True":
                        ...
                        if KonfluxImageBuilder.ART_FINALIZER in finalizers:
                            ...
                            pipelinerun_dict = pipelinerun.to_dict()
                            pipelinerun_dict['metadata']['finalizers'] = finalizers
                            await self._create_or_patch(dyn_client, pipelinerun_dict, patch_only=True)
